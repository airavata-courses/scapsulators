...
def get_satellite_view_data(message):
    """Parses the request-parameters, queries the Satellite_View_Reporter class, generates the parsed clean-dataset, and publishes a byte-stream of the report back via Kafka.
    Kafka-topic for reading requests of dataset: REQUEST_REPORT
    Kafka-topic for responding with parsed dataset: WEATHER_REPORT
    Sample REQUEST_REPORT body: {"visualize":"LWGNTICE", "timestamp":"2018-09-01 09:27:53"}
    Sample REQUEST_REPORT body: {"visualize":"ALBEDO", "timestamp":"2018-09-01 09:27:53"}
